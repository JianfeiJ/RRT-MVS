...
parser.add_argument('--dataset', default='general_eval4', help='select dataset')
...
parser.add_argument('--outdir', default='outputs/debug', help='output dir')
...
parser.add_argument('--max_h', type=int, default=832, help='testing max h')
...
# filter
parser.add_argument('--conf', type=float, default=0.55, help='prob confidence')
...
parser.add_argument('--dcn', default=False, help='dcn')
parser.add_argument('--arch_mode', type=str, default="fpn")
parser.add_argument('--rt', default=True, help='robust training')
...
parser.add_argument('--image_size', type=str, default=(832, 1152), help='training image size')
parser.add_argument('--pe', default=True, help='Positional Encoding')
...
def reproject_with_depth(depth_ref, intrinsics_ref, extrinsics_ref, depth_src, intrinsics_src, extrinsics_src):
    ...
    # conf = cv2.remap(src_confidence, x_src, y_src, interpolation=cv2.INTER_LINEAR)
    ...
    return depth_reprojected, x_reprojected, y_reprojected, x_src, y_src  # ,conf
...
def check_geometric_consistency(depth_ref, intrinsics_ref, extrinsics_ref, depth_src, intrinsics_src, extrinsics_src,
    ...
    dist_x = np.abs(x2d_reprojected - x_ref)
    dist_y = np.abs(y2d_reprojected - y_ref)
    ...
    for i in range(s, e):
        mask = np.logical_and(dist < i * dist_base, relative_depth_diff < math.log(max(i, 1.05), 10) * rel_diff_base)
        # mask = np.logical_and(dist < i * dist_base, relative_depth_diff < i * rel_diff_base)
        masks.append(mask)
    depth_reprojected[~mask] = 0
    x2d_reprojected[~mask] = 0
    y2d_reprojected[~mask] = 0

    return masks, mask, depth_reprojected, x2d_src, y2d_src, x2d_reprojected, y2d_reprojected
...
def filter_depth(pair_folder, scan_folder, out_folder, plyfilename):
    ...
    for ref_view, src_views in pair_data:
        ...
        for src_view in src_views:
            ...
            masks, geo_mask, depth_reprojected, x2d_src, y2d_src, x2d_reprojected, y2d_reprojected = check_geometric_consistency(
            ...
            all_re_x.append(x2d_reprojected)
            all_re_y.append(y2d_reprojected)

        depth_est_averaged = (sum(all_srcview_depth_ests) + ref_depth_est) / (geo_mask_sum + 1)
        depth_est_averaged[confidence > 0.75] = ref_depth_est[confidence > 0.75]
    ...
    with torch.no_grad():
        for batch_idx, sample in enumerate(TestImgLoader):
            ...
            for filename, cam, img, depth_est, photometric_confidence in zip(filenames, cams, imgs, \
                ...
                # depth_filename_reg = os.path.join(args.outdir, filename.format('depth_est_reg', '.pfm'))
                confidence_filename = os.path.join(args.outdir, filename.format('confidence', '.pfm'))
                cam_filename = os.path.join(args.outdir, filename.format('cams', '_cam.txt'))
                img_filename = os.path.join(args.outdir, filename.format('images', '.jpg'))
                ply_filename = os.path.join(args.outdir, filename.format('ply_local', '.ply'))
                os.makedirs(depth_filename.rsplit('/', 1)[0], exist_ok=True)
                # os.makedirs(depth_filename_reg.rsplit('/', 1)[0], exist_ok=True)
                ...
                # mi = np.min(depth_est_reg[depth_est_reg > 0])
                # ma = np.max(depth_est_reg)
                # depth = (depth_est_reg - mi) / (ma - mi + 1e-8)
                # depth = (255 * depth).astype(np.uint8)
                # depth_img_reg = cv2.applyColorMap(depth, cv2.COLORMAP_VIRIDIS)
                # cv2.imwrite(os.path.join(args.outdir, filename.format('depth_est_reg', '.png')), depth_img_reg)

                save_pfm(depth_filename, depth_est)
                # save_pfm(depth_filename_reg, depth_est_reg)
