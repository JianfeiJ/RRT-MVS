...
parser = argparse.ArgumentParser(description='A PyTorch Implementation of RRT-MVS')
...
# dataset
parser.add_argument('--dataset', default='blendedmvs', help='select dataset')
...
parser.add_argument('--image_size', type=str, default=(576, 768), help='training image size')
...
parser.add_argument('--lrepochs', type=str, default="6,8,10,12:2", help='epoch ids to downscale lr and the downscale rate')
parser.add_argument('--wd', type=float, default=0.0, help='weight decay')
parser.add_argument('--batch_size', type=int, default=1, help='train batch size')
parser.add_argument('--n_views', type=int, default=11, help='train views')
        ...
        for batch_idx, sample in enumerate(TrainImgLoader):
            ...
            if (not is_distributed) or (dist.get_rank() == 0):
                if do_summary:
                    ...
                    print(
                       "Epoch {}/{}, Iter {}/{}, lr {:.6f}, train loss = {:.3f}, ce_loss = {:.3f}, {:.3f}, {:.3f}, {:.3f}, range_err = {:.3f}, {:.3f}, {:.3f}, {:.3f}, epe:{:.6f}, err3:{:.6f}, err1:{:.6f},time = {:.3f}".format(
                           epoch_idx, args.epochs, batch_idx, len(TrainImgLoader),
                           optimizer.param_groups[0]["lr"], 
                           loss,
                           scalar_outputs["s0_ce_loss"],
                           scalar_outputs["s1_ce_loss"],
                           scalar_outputs["s2_ce_loss"],
                           scalar_outputs["s3_ce_loss"],
        ...
        if (not is_distributed) or (dist.get_rank() == 0):
            if (epoch_idx + 1) % args.save_freq == 0:
                ...
                torch.save({
                    ...
                    "{}/finalmodel_{}.ckpt".format(args.logdir,epoch_idx)) 
            ...
            for batch_idx, sample in enumerate(TestImgLoader):
                ...
                if (not is_distributed) or (dist.get_rank() == 0):
                    if do_summary:
                        ...
                        print(
                            "Epoch {}/{}, Iter {}/{}, lr {:.6f}, test loss = {:.3f}, c_loss = {:.3f}, {:.3f}, {:.3f}, {:.3f}, range_err = {:.3f}, {:.3f}, {:.3f}, {:.3f}, epe:{:.6f}, err3:{:.6f}, err1:{:.6f}, time = {:.3f}".format(
...
def train_sample(model, model_loss, optimizer, sample, args):
    ...
    loss, stage_ce_loss, range_err_ratio, epe, err3, err1 = model_loss(
                                        outputs, depth_gt_ms, mask_ms, sample_cuda["depth_min"], sample_cuda["depth_max"], stage_lw=[float(e) for e in args.dlossw.split(",") if e],
    ...
    scalar_outputs = {"loss": loss,
                      ...
                      "s1_ce_loss": stage_ce_loss[1],
                      "s2_ce_loss": stage_ce_loss[2],
                      "s3_ce_loss": stage_ce_loss[3],
...
if __name__ == '__main__':
    ...
    if args.dataset.startswith('dtu'):
        ...
        train_dataset = MVSDataset(args.trainpath, args.trainlist, "train", args.n_views, robust_train=args.rt)
        test_dataset = MVSDataset(args.testpath, args.testlist, "val", args.n_views)
    if is_distributed:
